...
import ImageListItemBar from "@mui/material/ImageListItemBar"; // for image uploaded state
...
export default class SNETImageUpload extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            outputImage:
                this.props.outputImageType === "url"
                    ...
                    : this.props.outputImage &&
                      SNETImageUpload.prepareBase64Image(this.props.outputImage),
        ...
        this.fileSizeError =
            "File size exceeds limits (" + this.props.maxImageSize / 1000000 + "mb).";
        this.fileTypeError =
            "File type not accepted. Allowed: " + this.props.allowedInputTypes + ".";
    ...
    static getDerivedStateFromProps(props, state) {
        if (props.outputImage === state.outputImage) {
            return null;
        }
        ...
        if (props.outputImage) {
            if (props.outputImageMimeType === undefined) {
                mimeType = SNETImageUpload.getBase64ImageType(props.outputImage);
            } else {
                mimeType = props.outputImageMimeType;
            }

            let outputImage;
            if (this.props.outputImageType === "url" || props.outputImage === "url") {
                outputImage = props.outputImage;
            } else {
                outputImage = SNETImageUpload.addBase64Header(mimeType, props.outputImage);
            }

            this.setState({
                displayModeTitle: props.displayModeTitle,
                outputImage,
                outputImageMimeType: mimeType,
                outputImageName: props.outputImageName,
                mainState: "display",
                value: props.disableOutputTab ? (props.disableComparisonTab ? 3 : 5) : 4,
            });
        } else {
            ...
            >
                ...
                >
                    <CloudUpload
                        style={
                            this.props.error
                                ? { fontSize: 48, color: snetRed }
                                : { fontSize: 48, color: this.mainColor }
                        }
                    />
                    <Typography
                        style={
                    ...
                    >
                        ...
                        are not saved on the servers after the job is processed.)
                    </Typography>
                </div>
            </div>
        );
        ...
        return (
            <div style={this.tabStyle}>
                <SwipeableViews axis="x" index={this.state.value}>
                    <div style={this.tabStyle}>{this.renderUploadTab()}</div>
                    <div style={this.tabStyle}>{this.renderUrlTab()}</div>
                    <div style={this.tabStyle}>{this.renderGalleryTab()}</div>
                    <div style={this.tabStyle}>
            ...
            <Fade in={this.state.mainState === "display"}>
                ...
                >
                    <img
                        ...
                        src={this.state.inputImageData}
                        onError={() =>
                            this.displayErrorMessage(this.inputImageErrorMessage, "display")
                        }
                        id="loadedImage"
                        // crossOrigin="anonymous"
                        style={
            ...
            <Fade in={this.state.mainState === "display"}>
                ...
                >
                    <img
                        ...
                        onError={() =>
                            this.displayErrorMessage(this.outputImageErrorMessage, "display")
                        }
                        id="loadedImage"
                        // crossOrigin="anonymous"
                        style={
...
SNETImageUpload.defaultProps = {
    ...
    allowedInputTypes: ["image/png", "image/jpeg", "image/jpg", "image/*"],
