...
const Visualizer = (props) => {
  ...
  const nodeTypes = props.graph.nodes
    .map((n) => n.data.type)
    .filter((s, i, arr) => {
      return arr.indexOf(s) === i && ["gene", "uniprot", "chebi", "reactome", "smp"].includes(s);
    });
  ...
  useEffect(() => {
    ...
    if (filteredElements) {
      ...
      cy.batch(() => cy.nodes().difference(filteredElements).style({ opacity: 0.1 }));
      cy.edges().difference(filteredElements).style({ opacity: 0 });
  ...
  const getAnnotationPercentage = (group, subgroup) => {
    ...
    filteredNodes = subgroup ? filteredNodes.filter((n) => n.data.type === subgroup) : filteredNodes;
  ...
  const assignColorToAnnotations = () => {
    const styles = AnnotationGroups.reduce((acc, annotation) => {
      const subGroupColors = annotation.subgroups.map((sg) => {
        return {
          selector: (n) => n.data().group.includes(annotation.group) && n.data().type === sg.name,
          style: {
