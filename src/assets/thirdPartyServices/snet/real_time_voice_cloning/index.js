...
export default class RealTimeVoiceCloningService extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...initialUserInput,
      users_guide: "https://singnet.github.io/dnn-model-services/users_guide/real-time-voice-cloning.html",
      code_repo: "https://github.com/singnet/dnn-model-services/tree/master/services/real-time-voice-cloning",
  ...
  canBeInvoked() {
    if ((this.state.audio || this.state.audio_url) && this.state.sentence) return true;
    return false;
  }
  ...
  submitAction() {
    const { audio, audio_url, sentence } = this.state;
    const methodDescriptor = RealTimeVoiceCloning["clone"];
    ...
    const props = {
      ...
      onEnd: response => {
        ...
        var blob_audio = new Blob([message.getAudio()], { type: "audio/wav" });
        this.setState({
          ...initialUserInput,
          response: {
            status: "success",
      ...
      <React.Fragment>
        <Grid container direction="column" justify="center" spacing={2}>
            ...
            <Grid item xs={12} style={{ textAlign: "center" }}>
              <div id="audio-container" className="col-md-3 col-lg-2">
                <audio controls style={{ height: "50px", width: "100%", fontSize: "13px", marginBottom: "5px" }}>
                  <source id="rtvcAudio" src="" type="audio/wav" />
                </audio>
              </div>
            </Grid>
          )}
          ...
          {this.props.isComplete && (
            <Grid item xs={12} style={{ textAlign: "center" }}>
              Response:
              <div id="response-container" className="col-md-3 col-lg-2">
                <audio controls style={{ height: "50px", width: "100%", fontSize: "13px", marginBottom: "5px" }}>
                  <source id="rtvcaAudio" src={this.state.response.audio} type="audio/wav" />
                </audio>
              </div>
            </Grid>
          )}
